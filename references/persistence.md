...
**A worker does not touch its service's RLS tables directly.** The stamp above comes from the inbound `IdentityContext` a request carries. A process that *originates* its own work — a Temporal worker, a scheduled job — carries none, so a transaction it opens against its own RLS tables stamps nothing and the policies admit no rows: a reconcile reads its own customer row back as absent. Do not paper over this by teaching the worker to stamp `service` itself (verifying its own token, a database decorator) — that is a second identity-to-database path maintained beside the interceptor's. The worker owns no database. It reaches its service's data the same way any other caller does: over gRPC, as a `service`, through operations the service exposes for it (`GetCustomerByStripeId`, `UpsertPurchase`, gated to `service`/`admin` per rule 37). The server interceptor then stamps `role: service` on the one path that already exists, and only `serve` holds a connection. See temporal-workflows.md — a worker holds service clients, not a `PostgresClient`.
