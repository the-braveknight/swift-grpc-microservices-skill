...
4. Keep the `Database` protocol in Core. Use `withConnection` for a single repository operation and `withTransaction` only for a multi-operation atomic use case. Do not hold a database transaction across an RPC; the sole exception is rotating a single-use secret, where a post-commit failure would destroy a valid credential, and only with an explicit deadline on the call.
