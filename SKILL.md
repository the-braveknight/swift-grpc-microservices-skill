...
36. Publish only what is called from outside the stack — which, with a tailnet sidecar owning the gateway's address, is nothing: run the gateway in the sidecar's network namespace and let it proxy to localhost. Databases and orchestrators get no `ports:`; where a host port must be published, make it a variable; run suite compose files from registry images, and declare required secrets as `${VAR:?message}` beside a `.env.example`.
...
7. Add `.env.example`, container build support, and a Compose service behind a tailnet sidecar that owns its address — no host port — or, without one, publishing the HTTP port alone.
